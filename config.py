...
# משקל = 0.5 ^ (גיל_בימים / מחצית-חיים). מכויל ב-Backtest: 270 ימים.
RECENCY_HALF_LIFE_DAYS = 270

# --- פרמטרי מודל Poisson / Dixon-Coles (מכוילים ב-Backtest על 2010-2022) ---
DC_RHO = 0.05             # תיקון Dixon-Coles לתוצאות נמוכות (כיול: 0.05)
ELO_BLEND_BETA = 0.3      # 1=רק התקפה/הגנה, 0=רק Elo (כיול: 0.3)
ELO_TO_GOALS = 0.004      # המרת פער Elo לעליונות שערים (כיול: 0.004)
MAX_GOALS = 8             # גודל מטריצת התוצאות (0..8 לכל צד)
MONTE_CARLO_RUNS = 5000   # מספר הרצות סימולציית הטורניר

# --- כוח סגל נוכחי (Current Team Strength) ---
CTS_WEIGHT = 0.3          # משקל כוח-הסגל מול ה-Elo ההיסטורי (0=בלי, 1=דומיננטי)
CTS_ELO_PER_SIGMA = 120.0 # כמה נק' Elo שווה סטיית-תקן אחת באיכות הסגל

# --- גורמים ברמת משחק (סעיפים 1-8) — משקלים קטנים ושקופים ---
FATIGUE_ELO_PER_DAY = 3.0    # נק' Elo להפרש יום מנוחה (מאומת: השפעה קטנה)
FATIGUE_MAX_DAYS = 5         # תקרת הפרש ימי מנוחה שנספרת
ALTITUDE_THRESHOLD_M = 1500  # מעל גובה זה — השפעה
ALTITUDE_PENALTY = 50.0      # קנס Elo לקבוצה לא-מותאמת-גובה (למשל במקסיקו סיטי)
H2H_MAX_DELTA = 20.0         # תקרת התאמת היסטוריית עימותים ישירים
H2H_SHRINK = 6.0             # ריכוך (מדגם קטן)
ODDS_BLEND = 0.5             # משקל מיזוג עם יחסי הימורים (אם הוזנו)
